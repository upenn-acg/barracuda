...
private:
	...
	bool parse_variable_value(PtxVariable& variable, Scope& s)
	{
		...
		variable.set_value(value.apply());
		scope.apply();
...
/*		return scope.one_of({
	...
	{
		std::shared_ptr<InsArgument> argument = parse_vector_argument(scope);
		if (!argument)
			return false;
	...
	std::shared_ptr<InsArgument> parse_vector_argument(Scope& scope)
	{
		const std::shared_ptr<InsArgument> empty;
	...
	bool parse_regular_indirect_argument(std::shared_ptr<PtxCodeLine>& stmt, Scope& scope)
	{
		...
		optional_parse_texture(scope, argument);
	...
	bool optional_parse_texture(Scope& s, std::shared_ptr<InsArgument>& arg)
	{
		...
		scope.skip_whitespace();
		Scope sampler(scope);
		std::string sampler_str;
		sampler.close_and_apply_if(parse_identifier(sampler, [&](auto& s)->bool {sampler_str = s; return true; })
			&& sampler.expect(TokenType::Punctuation, ","));
		scope.skip_whitespace();

		Scope tmp(scope);
		if (!tmp.expect(TokenType::Punctuation, "{"))
			return false;
		tmp.cancel();

		auto text_args = parse_vector_argument(scope);
		if (!text_args)
			return false;

		scope.apply();
		arg->set_texture_sampler(sampler_str);
		arg->set_texture_arg(text_args);
