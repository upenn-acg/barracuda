...
public:
    ...
    virtual int num_instructions() const { return 0; }
protected:
	DirType _dir_type;
...
{
public:
	PtxVariable() : PtxSymbol(DirType::Variable), _is_standalone(true), _array(NOT_ARRAY)
	{

	}

	PtxVariable(const std::string& type, const std::string& data_type, const std::string& name, bool standalone = true) : 
	...
	bool        is_array() const { return _array != NOT_ARRAY; }
	...
	void set_unknown_array() { _array = UNKNOWN_ARRAY; }
...
public:
    ...
    bool set_optional(const std::string& name)
    {
        _optional = name;
        return true;
    }

    bool has_optional() const { return _optional.size() > 0; }
    std::string optional() const { return _optional; }

	void set_texture_sampler(const std::string& arg)
	{
		_texture_sampler = arg;
	}

	void set_texture_arg(std::shared_ptr<InsArgument>& ptr)
	{
		_texture_arg = ptr;
	}
	...
	void print(std::ostream& out) const
	{
		...
		if (_texture_arg)
		{
			if (_texture_sampler.size() > 0)
			{
				out << ",";
				out << _texture_sampler;
			}

			out << ",";
			_texture_arg->print(out);
		}
...
private:
	...
	std::string _texture_sampler;
	std::shared_ptr<InsArgument> _texture_arg;
};
...
public:
    ...
    void clear_predicate() { _is_pred = _pred_neg = false; _pred = ""; }
    ...
    bool is_neg_predicated() const { return _pred_neg; }
    ...
    std::string modifier(int i, const char* def = "")
	{
		auto it = _modifiers.begin();
		for (; it != _modifiers.end() && i > 0; --i)
		{
			++it;
		}
    ...
    std::string modifier_from_list(const std::initializer_list<std::string>& items)
    {
        for(auto mit : _modifiers)
        {
            for(auto iit : items)
            {
                if(iit == mit)
	...
	{
		SS_UNKNOWN, SS_GLOBAL, SS_LOCAL, SS_PARAM, SS_SHARED, SS_CONST
	};
	...
	struct SSMapping
	{
		...
		static const inline std::initializer_list<SSMapping>& STATE_SPACES()
		{
			static const std::initializer_list<SSMapping> STATE_SPACES = {
				{ ".global", StateSpace::SS_GLOBAL },
                { ".local", StateSpace::SS_LOCAL },
                { ".param", StateSpace::SS_PARAM },
                { ".shared", StateSpace::SS_SHARED },
                { ".const", StateSpace::SS_CONST }
			};
			return STATE_SPACES;
		}
	};
	...
	bool prepend_argument(const std::shared_ptr<InsArgument>& value) { _arguments.push_front(value); return true; }
    ...
    std::string dst_register(bool only_direct = true)
    {
        if(_arguments.begin() != _arguments.end() && (*_arguments.begin())->type() == InsArgument::Type::Reg)
        {
            if (!(only_direct && (*_arguments.begin())->indirect()))
                return (*_arguments.begin())->name();
        }
        return std::string();
    }
    ...
    virtual int num_instructions() const override { return 1; }
private:
...
public:
    ...
    virtual int num_instructions() const
    {
        int count = 0;
        for(auto it = _elements.begin(); it != _elements.end(); ++ it)
        {
            if(*it)
                count += (*it)->num_instructions();
        }
        return count;
    }
...
public:
	...
	virtual void print(std::ostream& out) const override
	{
		...
		for (auto& dir : _header_directives)
			dir->print(out);
	...
	void prepend_arg(const std::shared_ptr<PtxVariable>& value) { _args.push_front(value); }
	...
	void add_header_directive(std::shared_ptr<PtxDirective>& header_directive)
	{
		_header_directives.push_back(header_directive);
	}
    ...
    virtual int num_instructions() const override
    {
        if(_main_block)
            return _main_block->num_instructions();
        return 0;
    }
...
private:
	...
	std::list<std::shared_ptr<PtxDirective>> _header_directives;
