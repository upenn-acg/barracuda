...
private:
	...
	bool instrument_func(const std::shared_ptr<PtxFunction>& func)
	{
		...
		for (auto& bb : analyzer)
		{
			...
			{
				if ((*it)->type() == PtxCodeElement::Type::TypeCode)
				{
					auto line = std::static_pointer_cast<PtxCodeLine>(*it);
					auto proto = get_proto(line);
					if (proto == OP_UNKNOWN)
						continue;
					int loc_id = 0;
					auto ins = create_instrumentation_instruction(proto, line, loc_id);
                    if(ins)
