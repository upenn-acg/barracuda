...
static timespec ts_start, ts_end;
...
__attribute__((destructor)) void __lib_destructor(void)
{
    ...
    const char* enabled = Env::dont_instrument_funcs() ? "DISABLED" : "ENABLED";

    if(start_set)
    {
        long time = (ts_end.tv_sec - ts_start.tv_sec) * 1000000000L;
        time += (ts_end.tv_nsec - ts_start.tv_nsec);
    	fprintf(stderr, "TIME MS %s %li %s\n", linkname, time / 1000000, enabled);
    }
    else
