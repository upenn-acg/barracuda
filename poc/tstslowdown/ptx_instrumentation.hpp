...
private:
	...
	std::shared_ptr<PtxBlock> create_instrumentation_instruction(PROTO_OP proto, std::shared_ptr<PtxCodeElement>& line, std::shared_ptr<PtxCodeLine> codeline, int loc_id)
	{
		...
		case OP_ATOMIC:
			...
			addr_value_name = add_instrumentation_target_addr(innerblock, codeline->get_state_space(), codeline->argument(1));
			create_instrumentation_instruction_no_addr(innerblock, addr_value_name, InsArgument::Type::Reg, proto, loc_id);
            innerblock->add(line);
            break;
		case OP_STORE:
            ss = codeline->get_state_space();
			if(!should_instrument(ss))
                return std::shared_ptr<PtxBlock>();
			addr_value_name = add_instrumentation_target_addr(innerblock, codeline->get_state_space(), codeline->argument(0));
			create_instrumentation_instruction_no_addr(innerblock, addr_value_name, InsArgument::Type::Reg, proto, loc_id);
            innerblock->add(line);
            break;
		case OP_SYNCTHREADS:
                                                ...
                                                const std::shared_ptr<InsArgument>& data_arg)
	{
		...
		auto decl = std::make_shared<PtxVariable>(".reg", ADDR_TYPE(), rsa);
		...
		case InsArgument::Type::Reg:
			...
			load_arg->add_argument(std::make_shared<InsArgument>(rsa, InsArgument::Type::Reg));
                ...
                InsArgument::Type addr_type, PROTO_OP proto, int loc_id)
	{
		...
		add_instrumented_param(block, ".b32", param_slim_locid, std::to_string(loc_id), InsArgument::Type::Immediate);
