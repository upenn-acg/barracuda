...
void Scanner::Init() {
	...
	for (i = 36; i <= 37; ++i) start.set(i, 2);
	for (i = 95; i <= 95; ++i) start.set(i, 2);
	for (i = 49; i <= 57; ++i) start.set(i, 8);
	start.set(48, 9);
	start.set(10, 10);
	start.set(46, 11);
		start.set(Buffer::EoF, -1);
	keywords.set(L"bla", 5);
	keywords.set(L"blu", 6);
