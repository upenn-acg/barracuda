...
private:
	...
	std::shared_ptr<PtxBlock> create_instrumentation_instruction(PROTO_OP proto, std::shared_ptr<PtxCodeLine>& line, bool predicated)
	{
		...
		case OP_ATOMIC:
			...
			return create_instrumentation_instruction_no_addr(block, addr_value_name, InsArgument::Type::Reg, proto, predicated);
		case OP_STORE:
            ss = line->get_state_space();
			if(!should_instrument(ss))
                return std::shared_ptr<PtxBlock>();
        ...
        case OP_CONVERGE:
        case OP_OTHER:
			return create_instrumentation_instruction_no_addr(block, NULL_STR, InsArgument::Type::Immediate, proto, predicated);
		default:
			assert(false);
			return std::shared_ptr<PtxBlock>();
		}
	...
	std::shared_ptr<PtxBlock> create_instrumentation_instruction_no_addr(
                ...
                InsArgument::Type addr_type, PROTO_OP proto, bool predicated)
	{
		...
		static const std::string param_slim_predicated = "param_slim_pred";
		...
		add_instrumented_param(block, ".b32", param_slim_predicated, std::to_string(predicated ? 1 : 0), InsArgument::Type::Immediate);
		...
		invoke->add_argument(std::make_shared<InsArgument>(param_slim_predicated, InsArgument::Type::Variable));
