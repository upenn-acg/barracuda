...
extern "C" __device__ __attribute__((noinline)) void STORE_OP_FUNCTION_NAME(uint64_t tid, const void* address, int op, int predicated)
{
    ...
    if(ldridx == myidx)
    {
        ...
        record->predicated = (uint16_t)predicated;
